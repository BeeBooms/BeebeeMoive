...
router.get("/", function(req, res){
    Print.find({},function(err, allPrints){
        if(err){
            ...
            res.render("landing.ejs",{prints: allPrints});
        }
    })
});
...
router.get("/checkbill", function(req, res){
    console.log(res.locals.currentUser);
    res.render("checkbill.ejs");
});
...
router.post('/register', upload.single('profileImage'), function(req, res){
    ...
    User.register(newUser, req.body.password, function(err, user){
        if(err){
            ...
            req.flash('success', user.username +', Welcome to BeeBeeMovie');
            res.redirect('/prints');;
            });
        }
    });
